...
use models\{CitaMedica, Configuracion, Especialidad, Especialidad_Medico, Notificaciones, Paciente, Persona, Programar_Horario, Servicio, TipoDocumento};
...

{
    ...
    public static function verProcedimientosMedico($id,$id_especialidad)
    {
      ...
      if(self::ValidateToken(self::get("token_")))
      {
        ...
        $Data = self::$ModelService->query()->Join("medico_especialidades as me","serv.id_medico_esp","=","me.id_medico_esp")
        ->Join("medico as m","me.id_medico","=","m.id_medico")
        ->Join("especialidad as es","me.id_especialidad","=","es.id_especialidad")
        ->Where("m.id_medico","=",$MedicoId)
        ->And("es.id_especialidad","=",$id_especialidad)
        ->And("deleted_at","is",null)->get();;  
        }else
        {
        $Data = self::$ModelService->query()->Where("id_medico_esp","=",$id)
        ->And("deleted_at","is",null)->get();
        }
    ...
    public static function citas_Programados(string $opcion,$fecha='')
    {
       ...
       if(self::ValidateToken(self::get("token_")))
       {
        ...
        if(self::profile()->rol === 'Médico'){
            $Citas_Programados = self::$Model->procedure("proc_citas_programados","c",[$opcion,$fecha,"Médico",self::MedicoData()->id_medico]);
        }else{
            $Citas_Programados = self::$Model->procedure("proc_citas_programados","c",[$opcion,$fecha,"Director",1]);
        }
    ...
    public static function citasMedicasCalendar()
    {
     ...
     { self::$Model = new CitaMedica;
        if(self::profile()->rol === self::$profile[1] || self::profile()->rol === self::$profile[0])
        {
            ...
            $data = self::$Model->procedure("proc_show_calendar_citas","C",[null]);
        }else{
            $MedicoIdentificador = self::MedicoData()->id_medico;
            $data = self::$Model->procedure("proc_show_calendar_citas","C",[$MedicoIdentificador]);
        }
        self::json($data);
     }
    }
