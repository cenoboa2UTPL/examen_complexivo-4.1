    ...
    columns:[
      ...
      {"data":null,render:function(nombreprod){
        if(parseInt(nombreprod.stock)>0 && ( parseInt(nombreprod.stock) <= parseInt(nombreprod.stock_minimo)))
        {
          ...
          if(parseInt(nombreprod.stock) === 0)
          {
...
function addCestaProducto(Tabla,Tbody)
{
  $(Tbody).on('click','#add_prod_venta',function(){
    ...
    $.ajax({
      url:RUTA+"app/farmacia/venta/add_cesta/"+DataId,
      method:"POST",
      dataSrc:"response",
      dataType:"json",
      success:function(response)
  ...
  $('#lista_detalle_venta_body').on('click','#quitar_producto_detalle',function(){
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $.ajax({
          url:RUTA+"app/farmacia/venta/quitar_producto_cesta",
          method:"POST",
          dataType:"json",
          data:{
   ...
   if(evento.which === 13)
    {
      ...
      if($(this).val().trim().length > 0)
      {
        ...
        $.ajax({
          url:RUTA+"app/farmacia/venta/update_cantidad__producto_cesta",
          method:"POST",
          data:{
          ...
          success:function(response){
            if(response.response === 'modificado')
            {
              ...
              if(response.response === 'stock-insuficiente')
              {
                Swal.fire({
                  title:"Mensaje del sistema!",
                  text:"La cantidad ingresada supera al stock del producto, ingrese una cantidad menor a su stock!",
                  icon:"error"
                }).then(function(){
                  showProductosDeLaCesta();
                });
...
{
  $.ajax({
    url:RUTA+"app/farmacia/venta/num_serie",
    method:"GET",
    dataType:"json",
    success:function(response){
     $('#serie').val(response.response);
    }
  });
}
